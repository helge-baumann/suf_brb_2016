...
fb_link <- data.frame(vars = names(rawdat), 
                      ...
                      kurz=as.integer(unlist(sapply(rawdat, get.lk, y=rawdat$langkurz, value=2)))
)

# nur die Fragen (ohne Order)
fb_lang <- fb_link[
  str_detect(fb_link[,1], fixed("Order", ignore_case=TRUE)) == F,2:3
  ]
...
for(i in which(!is.na(fb_df[,1]))) {
  ...
  if(str_detect(fb_df[i,1], "^[A-Z][0-9][[:alnum:]\\_]*|Intromail.:")) {
    ...
    laenge <- length(vars)
      ...
      if(laenge >  2) fb_df[i,3] <- paste(temp[c(1,length(temp))], collapse= " [...] ")
  ...
  # Codes rechtsbündig
  if(str_detect(fb_df[i,1], "^[:digit:]{1,6}:|^[:upper:]{1,2}:|^[:upper:][:lower:]{1,6}:") ) {
...
for(i in which(str_detect(fb_df[,2], "[A-Z][0-9]") & str_detect(fb_df[,1], "PROG:|WENN:"))) {
  ...
  for(b in x) {
    num <- num+1
      x[num] <- paste0("\\\\hyperref[", str_replace(b, fixed("_"), ":"),
                                     "]{", b, "}")
      y[1:2] <- paste(y[1:2], collapse=x[num])
      
                              
  }
  
  fb_df[i,2] <- paste(paste(y, x, sep=""), collapse="")
}
